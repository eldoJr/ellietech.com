...
const getGlowColor = () =>
  getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#a5b4fc";
...
const DotField = memo(({
  ...
  const glowStopEndRef = useRef<SVGStopElement>(null);
  ...
  useEffect(() => {
    ...
    function syncGlowColor() {
      ...
      if (glowStopRef.current) glowStopRef.current.setAttribute("stop-color", color);
      if (glowStopEndRef.current) glowStopEndRef.current.setAttribute("stop-color", color);
    }
    ...
    function tick() {
      ...
      ctx!.fillStyle = isDark ? "rgba(165,180,252,0.22)" : "rgba(79,70,229,0.15)";
      ...
      <svg ref={svgRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
        <defs>
          <radialGradient id={glowIdRef.current}>
            <stop ref={glowStopRef} offset="0%" stopColor={getGlowColor()} stopOpacity="1" />
            <stop ref={glowStopEndRef} offset="100%" stopColor={getGlowColor()} stopOpacity="0" />
          </radialGradient>
        </defs>
        <circle
          ref={glowRef}
